...
class GenerationMixin:
    ...
    def _prepare_input_ids_for_generation(self, bos_token_id: int) -> torch.LongTensor:
        ...
        return tch.ones((1, 1), dtypeor=torch.long, device=self.device) * bos_token_id
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(
        self, input_ids: torch.LongTensor,en_labels: torch.LongTensor, model_kwargs
    ) -> Dict[str, Any]:
        ...
        model_kwargs["encoder_outputs"]: ModelOutput = encoder(input_ids,en_labels=en_labels, return_dict=True, **encoder_kwargs)
        return model_kwargs

    def _prepare_decoder_input_ids_for_generation(
        self, input_ids: torch.LongTensor, decoder_start_token_id: int = None, bos_token_id: int = None,target_token_id: int = None
    ) -> torch.LongTensor:
        decoder_start_token_id = self._get_decoder_start_token_id(decoder_start_token_id, bos_token_id)
        decoder_input_ids = (
            torch.ones((input_ids.shape[0], 2), dtype=input_ids.dtype, device=input_ids.device)
            * decoder_start_token_id
        )
        decoder_input_ids[:,1]=target_token_id
    ...
    def _expand_inputs_for_generation(
        ...
        de_inputs_labels: torch.LongTensor,
        expand_size: int = 1,
        ...
        **model_kwargs,
        ...
        de_inputs_labels =de_inputs_labels.index_select(0, expanded_return_idx)
        ...
        return input_ids,de_inputs_labels, model_kwargs
    ...
    def generate(
        ...
        feature_vocab_dict_list=None,
        en_labels : Optional[torch.LongTensor] = None,
        **model_kwargs,
        ...
        if self.config.is_encoder_decoder:
            ...
            model_kwargs = self._prepare_encoder_decoder_kwargs_for_generation(input_ids,en_labels, model_kwargs)
            ...
            if "decoder_input_ids" in model_kwargs:
                ...
                input_ids = self._prepare_decoder_input_ids_for_generation(
                    input_ids, decoder_start_token_id=decoder_start_token_id, bos_token_id=bos_token_id,target_token_id=target_token_id
                )
                de_inputs_labels = (
                    torch.ones((input_ids.shape[0], 2), dtype=input_ids.dtype, device=input_ids.device)
                    * 4
                )
            # print(de_inputs_labels.size(),'ll')
        ...
        if is_greedy_gen_mode:
            ...
            return self.greedy_search(
                ...
                de_inputs_labels=de_inputs_labels,
                **model_kwargs,
            )
            ...
            input_ids,de_inputs_labels, model_kwargs = self._expand_inputs_for_generation(
                input_ids,de_inputs_labels, expand_size=num_beams, is_encoder_decoder=self.config.is_encoder_decoder, **model_kwargs
            )
            # print(input_ids,input_ids.size())
            # input_ids:batch*beams,sequencelen
            # print(de_inputs_labels.size(),'ls')
            return self.beam_search(
                ...
                feature_vocab_dict_list=feature_vocab_dict_list,
                de_inputs_labels=de_inputs_labels,
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        while cur_len < max_length:
            ...
            if cur_len >= 2:
                de_inputs_labels_ = de_inputs_labels[:,-1]
                de_inputs_labels_ = de_inputs_labels_.unsqueeze(-1)
            ...
            outputs = self(
                **model_inputs,
                ...
                de_labels_ids=de_inputs_labels_,
            )
            next_token_logits = outputs.logits[:, -1, :]
            next_label_logits = outputs.logits_l[:, -1, :]

            decoder_output_label=torch.argmax(next_label_logits,dim=-1)
            decoder_output_label=decoder_output_label.unsqueeze(-1)

            next_label=torch.empty(input_ids.size(0),1, dtype=torch.long).fill_(4).cuda()
            ...
            for index,i in enumerate(input_ids[:,-1]):
                ...
                if cur_len >= 2:
                    ...
                    elif (aspect_level[index]==0 and decoder_output_label[index].item() in inside_label_index) or (aspect_level[index]!=0 and decoder_output_label[index].item() in inside_label_index and label_dict_reserve[decoder_output_label[index].item()][2:]!=label_dict_reserve[de_inputs_labels_[index].item()][2:]) :
                        decoder_output_label_mask = torch.zeros(next_label_logits[index].size()).cuda()
                        decoder_output_label_mask[begin_label_index]=1
                        decoder_output_label_ = torch.mul(torch.exp(next_label_logits[index]),decoder_output_label_mask)
                            
                        normalize = decoder_output_label_.sum(dim=-1,keepdim=True)
                
                        decoder_output_label_ = decoder_output_label_ / normalize
                        decoder_output_label_ = torch.argmax(decoder_output_label_,dim=-1)
                        next_label[index]=decoder_output_label_
                ...
                if next_label[index].item() in label_index_list:       
                    ...
                    for j,index_list in enumerate(asp_label_index_list):
                        ...
                        if next_label[index].item() in index_list:
                            if new_aspect_category[index]!=j:
            ...
            de_inputs_labels = torch.cat([de_inputs_labels, next_label], dim=-1)
        ...
        if return_dict_in_generate:
            ...
            # print(input_ids.size(),de_inputs_labels.size())
            return input_ids,de_inputs_labels
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        new_aspect_category=torch.empty(input_ids.size(0),1, dtype=torch.long).fill_(-1).cuda()
        n_no_aspect=torch.empty(input_ids.size(0),1, dtype=torch.long).fill_(0).cuda()
        aspect_level=torch.empty(input_ids.size(0),1, dtype=torch.long).fill_(0).cuda()
        pre_aspect=torch.empty(input_ids.size(0),1, dtype=torch.long).fill_(-1).cuda()
        ...
        while cur_len < max_length:
            ...
            import copy
            next_token_logits_=copy.copy(next_token_logits)
            # adjust tokens for Bart, *e.g.*
            decoder_output_label=torch.argmax(next_label_logits,dim=-1)
            decoder_output_label=decoder_output_label.unsqueeze(-1)

            # decoder_output_label=next_label_logits
            # print(decoder_output_label.size())
            ...
            flag=torch.empty(input_ids.size(0),1, dtype=torch.long).fill_(0).cuda()
            next_label=torch.empty(input_ids.size(0),1, dtype=torch.long).fill_(4).cuda()
            ...
            for index,i in enumerate(input_ids[:,-1]):
                ...
                if cur_len >= 4:
                    # print(decoder_output_label[index].item())
                    if aspect_level[index]==0 and (decoder_output_label[index].item() in begin_label_index):
                        
                        next_label[index]=   decoder_output_label[index]  
                        # decoder_output_label = decoder_output_label.squeeze(0)
                        # print(decoder_output_label.size())

                    elif (aspect_level[index]==0 and decoder_output_label[index].item() in inside_label_index) or (aspect_level[index]!=0 and decoder_output_label[index].item() in inside_label_index and label_dict_reserve[decoder_output_label[index].item()][2:]!=label_dict_reserve[de_inputs_labels_[index].item()][2:]) :
                        # print(decoder_output_label.size())
                        decoder_output_label_mask = torch.zeros(next_label_logits[index].size()).cuda()
                        # print(decoder_output_label_mask.size())
                        decoder_output_label_mask[begin_label_index]=1
                        decoder_output_label_ = torch.mul(torch.exp(next_label_logits[index]),decoder_output_label_mask)
                ...
                if next_label[index].item() in label_index_list:
                    ...
                    for j,index_list in enumerate(asp_label_index_list):
                        ...
                        if next_label[index].item() in index_list:
                            list_dict=feature_vocab_dict_list[j]
                            if new_aspect_category[index]!=j:
                                aspect_level[index]=0
                                pre_aspect[index]=-1
                            new_aspect_category[index]=j
                    ...
                    # if aspect_level[index]<len(list_dict):
                    #     decoder_output_mask = torch.zeros(next_token_logits[index].size()).cuda()
                    #     decoder_output_mask[list_dict[aspect_level[index].item()][pre_aspect[index].item()]]=1
                    #     # n_next_aspects=len(list_dict[aspect_level][index])
                        
                    #     next_token_logits[index,:]=torch.mul(torch.exp(next_token_logits[index,:]),decoder_output_mask)
                    #     next_token_logits[index,:]+=1e-12
                    #     normalize = next_token_logits[index,:].sum(dim=-1,keepdim=True)
                    #     next_token_logits[index,:] = next_token_logits[index,:] / normalize
                    #     next_token_logits[index,:] =torch.log(next_token_logits[index,:])
                        
                    #     # if n_next_aspects<num_beams:
                    #     #     exsit_list=[]
                    #     #     for k in num_beams-n_next_aspects:
                    #     #         decoder_output=F.log_softmax(next_token_logits_[index,:], dim=-1) 
                    #     #         log_prob, indices = torch.topk(decoder_output, num_beams)
                    #     #         for feauture_index in indices:
                    #     #             if feauture_index not in list_dict[aspect_level[index] not in exsit_list:
                    ...
                    # else:
                    #     # n_next_aspects = 0 
                    #     next_token_logits[index,:]=F.log_softmax(next_token_logits_[index,:], dim=-1) 
            ...
            #     print(index,aspect_level[index],pre_aspect[index],i.item(),new_aspect_category[index],flag[index])
            # print('................................')
            # if {"<B-OP>": 50269,  "<B-ASP>": 50267, "<I-ASP>": 50268, "<I-OP>": 50270}
            # next_token_scores = F.log_softmax(next_token_logits_, dim=-1)  # (batch_size * num_beams, vocab_size)
            ...
            # print('next_token_scores',next_token_scores.size())
            ...
            n_no_aspect = n_no_aspect.index_select(0, beam_idx.view(-1))
            aspect_level = aspect_level.index_select(0, beam_idx.view(-1))
            pre_aspect = pre_aspect.index_select(0, beam_idx.view(-1))
            flag=flag.index_select(0, beam_idx.view(-1))
            new_aspect_category=new_aspect_category.index_select(0, beam_idx.view(-1))
            next_label=next_label.index_select(0, beam_idx.view(-1))

            # print(beam_scores)
            # print(beam_idx)
            # print(beam_idx.size())
            # print('input_idsbeam_next_tokens',input_ids.size(),beam_next_tokens.size())
            input_ids = torch.cat([input_ids[beam_idx, :], beam_next_tokens.unsqueeze(-1)], dim=-1)
            de_inputs_labels = torch.cat([de_inputs_labels[beam_idx, :], next_label], dim=-1)
            for index,i in enumerate(input_ids[:,-1]):
                if flag[index]==1:
                    pre_aspect[index]=i.item()
            # print('input_ids',input_ids.size())
            cur_len = cur_len + 1
        ...
        sequence_outputs = beam_scorer.finalize(
            input_ids,de_inputs_labels, beam_scores, next_tokens, next_indices, pad_token_id=pad_token_id, eos_token_id=eos_token_id,
        )
        ...
        if return_dict_in_generate:
            ...
            return sequence_outputs["sequences"],sequence_outputs["sequences_label"]
