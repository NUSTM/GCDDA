            ...
            for beam_token_rank, (next_token, next_score, next_index) in enumerate(
                ...
                if (eos_token_id is not None) and (next_token.item() == eos_token_id):
                    ...
                    beam_hyp.add3(
                        ...
                        next_score.item(),
                        de_inputs_labels[batch_beam_idx].clone(),
    ...
    def finalize(
        ...
        for batch_idx, beam_hyp in enumerate(self._beam_hyps):
            ...
            for beam_id in range(self.num_beams):
                ...
                final_labels = de_inputs_labels[batch_beam_idx]
                beam_hyp.add3(final_tokens, final_score,final_labels)
        # print(beam_hyp)
        ...
        for i, beam_hyp in enumerate(self._beam_hyps):
            ...
            for j in range(self.num_beam_hyps_to_keep):
                ...
                best_hyp_label = best_hyp_tuple[2]
                sent_lengths[self.num_beam_hyps_to_keep * i + j] = len(best_hyp)

                # append to lists
                best.append((best_hyp,best_hyp_label))
                best_scores[i * self.num_beam_hyps_to_keep + j] = best_score

        # prepare for adding eos
        sent_max_len = min(sent_lengths.max().item() + 1, self.max_length)
        decoded: torch.LongTensor = input_ids.new(batch_size * self.num_beam_hyps_to_keep, sent_max_len)
        decoded_labels: torch.LongTensor = input_ids.new(batch_size * self.num_beam_hyps_to_keep, sent_max_len)
        # shorter batches are padded if needed
        if sent_lengths.min().item() != sent_lengths.max().item():
            assert pad_token_id is not None, "`pad_token_id` has to be defined"
            decoded.fill_(pad_token_id)
            decoded_labels.fill_(4)

        # fill with hypotheses and eos_token_id if the latter fits in
        for i, (hypo,label) in enumerate(best):
            decoded[i, : sent_lengths[i]] = hypo
            decoded_labels[i, : sent_lengths[i]] = label
            if sent_lengths[i] < self.max_length:
                decoded_labels[i, sent_lengths[i]] = 4
                decoded[i, sent_lengths[i]] = eos_token_id
        return UserDict(
            {
                "sequences": decoded,
                "sequence_scores": best_scores,
                "sequences_label": decoded_labels,
            }
        )
...
class BeamHypotheses:
    ...
    def add3(self, hyp: torch.LongTensor, sum_logprobs: float,hyp_label: torch.LongTensor,):
        """
        Add a new hypothesis to the list.
        """
        score = sum_logprobs / (hyp.shape[-1] ** self.length_penalty)
        if len(self) < self.num_beams or score > self.worst_score:
            self.beams.append((score, hyp,hyp_label))
            if len(self) > self.num_beams:
