...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def set_input_embeddings(self, new_embeddings):
        ...
        self.encoder.set_input_embeddings(new_embeddings)
        self.decoder.set_input_embeddings(new_embeddings)
        # self.decoder_t.set_input_embeddings(new_embeddings)
    ...
    def forward(
        ...
        if labels is not None and decoder_input_ids is None and decoder_inputs_embeds is None:
            # get decoder inputs from shiftin g lm labels to the right
        ...
        # decoder_outputs_t = self.decoder_t(
        #     input_ids=decoder_input_ids,
        #     attention_mask=decoder_attention_mask,
        #     inputs_embeds=decoder_inputs_embeds,
        #     past_key_values=past_key_values,
        ...
        # sequence_output_t = decoder_outputs_t[0]

        # Set device for model parallelism
        if self.model_parallel:
            torch.cuda.set_device(self.encoder.first_device)
            self.lm_head = self.lm_head.to(self.encoder.first_device)
            # self.lm_head_t = self.lm_head_t.to(self.encoder.first_device)
            sequence_output = sequence_output.to(self.lm_head.weight.device)
            # sequence_output_t=sequence_output_t.to(self.lm_head.weight.device)
        ...
        if self.config.tie_word_embeddings:
            ...
            # sequence_output_t = sequence_output_t * (self.model_dim ** -0.5)
        
        lm_logits = self.lm_head(sequence_output)
        # lm_logits_t = self.lm_head_t(sequence_output_t)
        ...
        if labels is not None:
            ...
            # target = (labels[:,0] == 32101).cpu().data
            ...
            # lm_logits[sent_idx,:,:]=lm_logits_t[sent_idx,:,:]
