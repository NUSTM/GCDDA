...
def train(args):
    ...
    train_examples = processor.get_train_examples_da_train(args.data_dir,args.domain_pair,args.dataset_split, args.task_type)
...
def test(args):  # Load a trained model that you have fine-tuned
    ...
    label_list_map = dict(zip([i for i in range(len(label_list))], label_list))
    tokenizer = ABSATokenizer.from_pretrained(modelconfig.MODEL_ARCHIVE_MAP[args.bert_model])
    eval_examples = processor.get_test_examples_da_test(args.data_dir,args.domain_pair,args.dataset_split, args.task_type)
