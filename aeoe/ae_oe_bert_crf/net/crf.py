...
class CRF(nn.Module):
    ...
    #         print(sum.size(),forward.size(), index.size())
    ...
    def viterbi(self, features):
        time_steps = features.size(0)
        forward = Variable(self.transitions[self.start_tag, :self.start_tag].cpu()+features[0,:])  # START_TAG
        if self.use_cuda:
            forward = forward.cuda()
        # back_points 到该点的最大分数  last_points 前一个点的索引

        
        back_points, index_points = [self.transitions[self.start_tag, :self.start_tag].cpu()+features[0,:]], [torch.LongTensor([-1]).expand_as(forward)]
