...
def tag2aspect(tag_sequence,isasp):
    """
...
def evaluate_chunk(test_Y, pred_Y,isasp):
    """
    ...
    for i in range(length):
        ...
        gold_aspects = tag2aspect(gold,isasp)
        pred_aspects = tag2aspect(pred,isasp)
...
def eval_result(dir):
    ...
    print('属性抽取: {} recall: {} f1: {}'.format(p, r, f1))
...
def eval_ts_result(dir):
    input_file = os.path.join(dir, 'pre.txt')
    ad_output_file = os.path.join(dir, 'subtask_ae_eval_result.txt')
    ads_output_file = os.path.join(dir, 'absa_eval_result.txt')
    with open(input_file, 'r', encoding='utf-8') as fp:
        lines = fp.readlines()
    dataset = []
    test_Y, pred_Y = [], []
    ...
    ad_p, ad_r, ad_f1 = evaluate_chunk(test_Y, pred_Y,1)
    ado_p, ado_r, ado_f1 = evaluate_chunk(test_Y, pred_Y,2)
    # ads_p, ads_r, ads_f1 = evaluate_absa_chunk(test_Y, pred_Y)
    print('属性抽取: {} recall: {} f1: {}'.format(ad_p, ad_r, ad_f1))
    print('联合任务： precision: {} recall: {} f1: {}'.format(ado_p, ado_r, ado_f1))
    ...
    with open(ads_output_file, 'w', encoding='utf-8') as fp:
        fp.write('P: ' + str(ado_p) + ' R: ' + str(ado_r) + ' f1: ' + str(ado_f1) + '\n')
    return ado_f1


def print_valid_result(dir):
    input_file = os.path.join(dir, 'pre.txt')
